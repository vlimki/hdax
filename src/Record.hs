...
module Record (Record (..), convert, Value (..), CsvField, valueToR, get, del, fieldMaybe, toValue, isNull, set, getValue) where
...
getValue :: T.Text -> Record -> Value
getValue s r = fromMaybe (error "Invalid field") $ M.lookup s $ inner r

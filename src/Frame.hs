...
module Frame (readCsv, Frame, Value, col, row, Series, cols, rows, toHMatrix, (!>), field, Frame.filter) where
...
rows idxs (Frame h r) = Frame{headers=h, records=V.backpermute r $ V.fromList idxs}
